...
var Actor = Box.extend({
    ...
    processInput: function (direction) {
        ...
        if (anyIn(keys.up, direction)) {
            if (!this.isInAir() || this.MULTI_JUMP > this.numJumps || this.MULTI_JUMP == -1) {
                var now = App.physicsTimeElapsed;
                if (now - this.lastJump > this.JUMP_DELAY) { // press jump again
    ...
    dampVelocity: function () {
        if (this.DAMPING_FACTOR !== null && !App.Utils.almostEqual(this.xVelocity, 0, 0.0001)) {
